...
export default function FollowingPage() {
  ...
  const fetchFollowing = useCallback(
    async (nextCursor?: string) => {
      if (!user?.id) return;
      try {
        const result = await usersApi.getFollowing(user.id, nextCursor);
        setFollowing((prev) =>
        ...
        console.error(ApiError.getMessage(err, 'Failed to load following'));
      } finally {
        setIsLoading(false);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-gray-900">我的追蹤</h1>
      </div>
      ...
      {!isLoading && following.length > 0 && (
        <div className="space-y-3">
          {following.map((item) => {
            const initials = (item.displayName || item.username || 'U')
              .slice(0, 2)
              .toUpperCase();
            return (
      ...
      {!isLoading && following.length === 0 && (
        <div className="flex flex-col items-center py-16 text-center">
          ...
          <h2 className="text-lg font-semibold text-gray-900">還沒有追蹤任何人</h2>
