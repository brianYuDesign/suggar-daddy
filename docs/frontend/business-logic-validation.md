...
**位置**: 
- `apps/web/app/(main)/subscription/page.tsx`
...
### 7.3 風險 3: Admin 授權繞過風險 ✅

**位置**: `apps/admin/` 整體

#### 已實施的修復

1. **✅ Next.js Middleware 權限檢查**
   
   **文件**: `apps/admin/middleware.ts`
...
3. **✅ 權限管理 Hook**
   ...
   **文件**: `apps/admin/lib/permissions.ts`
   
   功能：
   - ✅ 細粒度權限控制
   - ✅ 角色權限映射
   - ✅ 權限檢查 Hook（usePermissions）
   - ✅ 權限守衛 HOC（withPermission）
   - ✅ 超級管理員判斷
...
| 風險 | 狀態 | 修復項目 | 測試案例 | 實際工作量 |
|------|------|----------|----------|------------|
| 風險 1: 提款驗證 | ✅ | 5 項 | 10 個 | 8 小時 |
| 風險 2: 幂等性 | ✅ | 5 項 | 9 個 | 8 小時 |
| 風險 3: Admin 授權 | ✅ | 5 項 | 11 個 | 7 小時 |
| **總計** | **✅** | **15 項** | **30 個** | **23 小時** |

#### 新增文件

1. `apps/admin/middleware.ts` - Admin 路由保護中介軟體
2. `apps/admin/lib/permissions.ts` - 權限管理系統
3. `apps/web/app/(main)/wallet/withdraw/page.test.tsx` - 提款測試
4. `apps/web/app/(main)/subscription/page.test.tsx` - 訂閱測試
5. `apps/admin/middleware.test.ts` - Middleware 測試

#### 修改文件

1. `apps/web/app/(main)/wallet/withdraw/page.tsx` - 提款頁面
2. `apps/web/app/(main)/subscription/page.tsx` - 訂閱頁面
3. `apps/admin/lib/auth.ts` - Token 驗證增強
