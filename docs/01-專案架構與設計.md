...
| user-service | 3001 | `/api/v1/users` | 用戶資料、個人頁面、推薦卡片 |
| auth-service | 3002 | `/api/v1/auth` | JWT 認證、註冊登入、Refresh Token |
| matching-service | 3003 | `/api/v1/matching` | 滑動、推薦卡片、配對 |
| notification-service | 3004 | `/api/v1/notifications` | 推播通知（Kafka-driven） |
| messaging-service | 3005 | `/api/v1/messaging` | 對話與訊息（Kafka-driven） |
...
| admin-service | 3011 | `/api/v1/admin` | 管理後台 API（ADMIN only） |
