...
- 路徑對照：`/api/v1/auth` → auth、`/api/v1/users` → user、`/api/v1/matching` → matching、`/api/posts` → content、`/api/subscriptions` → subscription、`/api/tips`、`/api/stripe` → payment、`/api/upload`、`/api/media` → media、`/api/v1/admin` → admin-service。
- 可透過環境變數覆寫各服務 URL。
...
- `getCards` 呼叫 user-service **GET /api/v1/users/cards**（exclude 自己 + 已 swipe），回傳真實用戶卡片。
